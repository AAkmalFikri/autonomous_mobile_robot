...
bool LslidarN301Decoder::loadParameters() {
    pnh.param<int>("point_num", point_num, 100000);
    pnh.param<double>("min_range", min_range, 0.1);
...
void LslidarN301Decoder::publishScan()
{
    ...
    for(uint16_t j=0;j < SCANS_PER_FIRING-1;j++)
    {

      for(uint16_t i = 0; i < sweep_data->scans[j].points.size()-1; i++)
      {
...
void LslidarN301Decoder::decodePacket(const RawPacket* packet) {
    ...
    for (size_t fir_idx = 1; fir_idx < FIRINGS_PER_PACKET; fir_idx+=2) {
        ...
        firings[fir_idx].firing_azimuth =
            firings[fir_idx].firing_azimuth > 2*M_PI ? firings[fir_idx].firing_azimuth-2*M_PI:firings[fir_idx].firing_azimuth;
        firings[fir_idx].firing_azimuth =
            firings[fir_idx].firing_azimuth < 0 ? firings[fir_idx].firing_azimuth + 2*M_PI:firings[fir_idx].firing_azimuth;
    }
    ...
    for (size_t blk_idx = 0; blk_idx < BLOCKS_PER_PACKET; ++blk_idx) {
        ...
        for (size_t blk_fir_idx = 0; blk_fir_idx < FIRINGS_PER_BLOCK; ++blk_fir_idx){
            ...
            for (size_t scan_fir_idx = 0; scan_fir_idx < SCANS_PER_FIRING; ++scan_fir_idx){
				...
				if (scan_fir_idx < SCANS_PER_FIRING-1)
				{
	                // Azimuth
				...
				else
				{
					...
					if (blk_fir_idx == 0)
					{ 
						pTime.tm_year = raw_block.data[byte_idx]+2000-1900;
						pTime.tm_mon = raw_block.data[byte_idx+1]-1;
						pTime.tm_mday = raw_block.data[byte_idx+2];
					}
					else
					{
						pTime.tm_hour = raw_block.data[byte_idx];
						pTime.tm_min = raw_block.data[byte_idx+1];
						pTime.tm_sec = raw_block.data[byte_idx+2]+1;
					}
				}
            }
...
void LslidarN301Decoder::packetCallback(
    ...
    for (size_t fir_idx = 0; fir_idx < FIRINGS_PER_PACKET; ++fir_idx) {
        for (size_t scan_idx = 0; scan_idx < SCANS_PER_FIRING-1; ++scan_idx) {
